...
export default function AILoadingState({ taskSequences }: AILoadingStateProps) {
    ...
    const currentSequence = taskSequences[sequenceIndex];
    ...
    useEffect(() => {
        const advanceTimer = setInterval(() => {
            ...
            if (nextLineIndex < firstVisibleLineIndex && nextLineIndex !== 0) {
                setSequenceIndex(
                    (prevIndex) => (prevIndex + 1) % taskSequences.length
                );
                return;
            }
        ...
        <div className="flex items-center justify-center min-h-full w-full">
            <div className="space-y-4 w-auto">
                <div className="ml-2 flex items-center space-x-2 text-gray-600 dark:text-gray-300 font-medium">
                    <LoadingAnimation
                        progress={(sequenceIndex / taskSequences.length) * 100}
                    />
