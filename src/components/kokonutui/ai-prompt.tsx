...
export default function AI_Prompt({ onSubmit, hideBottomTools = false, placeholder }: AIPromptProps) {
    ...
    const MODE_ICONS: Record<string, React.ReactNode> = {
        "text": <Bot className="w-4 h-4 opacity-50" />,
        "spreadsheet": <Paperclip className="w-4 h-4 opacity-50" />,
    };
    ...
    const handleSubmit = () => {
        if (selectedMode === 'text' && value.trim()) {
            onSubmit(value, selectedMode);
            setValue('');
            if (textareaRef.current) {
                adjustHeight(true);
            }
        } else if (selectedMode === 'spreadsheet' && file) {
    ...
    return (
        <div className="w-full py-4">
            <div className="bg-zinc-100 dark:bg-zinc-800 rounded-2xl p-1.5 pt-4">
                    ...
                    <div className="relative flex flex-col">
                        ...
                        >
                            <Textarea
                                id="ai-input-15"
                                value={value}
                                placeholder={placeholder || (selectedMode === 'Text' ? "What can I do for you?" : "Select a spreadsheet file below")}
                                disabled={selectedMode === 'Spreadsheet'}
                                className={cn(
                                    "w-full rounded-xl rounded-b-none px-4 py-3 bg-zinc-200 dark:bg-zinc-800 border-none dark:text-white placeholder:text-black/70 dark:placeholder:text-white/70 resize-none focus-visible:ring-0 focus-visible:ring-offset-0",
                                    "min-h-[72px]"
                                )}
                                ref={textareaRef}
                        ...
                        <div className="h-14 bg-zinc-200 dark:bg-zinc-800 rounded-b-xl flex items-center">
                            <div className="absolute left-3 right-3 bottom-3 flex items-center justify-between w-[calc(100%-24px)]">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <label
                                            className={cn(
                                                ...
                                                selectedMode === 'Spreadsheet' ? "text-blue-500 bg-blue-500/10" : "text-black/40 dark:text-white/40 hover:text-black dark:hover:text-white"
                                            )}
                                            aria-label="Attach file"
                                        >
                                            <input type="file" className="hidden" disabled={selectedMode !== 'spreadsheet'} onChange={(e) => setFile(e.target.files?.[0] || null)} />
                                ...
                                <button
                                    ...
                                    className={cn(
                                        ...
                                        "hover:bg-black/10 dark:hover:bg-white/10 focus-visible:ring-1 focus-visible:ring-offset-0 focus-visible:ring-blue-500",
                                        hideBottomTools && "ml-auto"
                                    )}
                                    aria-label="Send message"
                                    disabled={selectedMode === 'text' ? !value.trim() : !file}
