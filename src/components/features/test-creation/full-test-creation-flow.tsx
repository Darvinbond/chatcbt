...
interface FullTestCreationFlowProps {
  folderId?: string;
  placeholder?: string;
  topTitle?: ReactNode;
...
export function FullTestCreationFlow({
  folderId,
  placeholder,
  topTitle,
  onModeChange
  ...
  // Notify parent component about mode changes
  useEffect(() => {
    onModeChange?.(!showHeader); // isChatMode = !showHeader
  }, [showHeader, onModeChange]);
  ...
  // Auto-scroll to bottom whenever messages length increases
  useEffect(() => {
  ...
  const createTestMutation = useMutation({
    ...
    onSuccess: (data, variables) => {
      ...
      // Refresh sidebar and redirect immediately
      refresh();
  ...
  return (
    <div className={cn(`relative`, showHeader ? "" : "flex-1")}>
      <div
        className={cn(`flex flex-col relative`, showHeader ? "h-full" : "")}
      >
        {/* Top content area - only show messages when header is hidden */}
        {!showHeader && (
          <div className={cn(
            "flex-1 min-h-full w-full max-w-2xl mx-auto",
            folderId ? "pb-6" : "pb-[80px]"
          )}>
            <ChatMessages
              messages={messages}
        ...
        {/* Input area - dynamic positioning based on showHeader */}
        <div className={cn(
          "p-4 bg-background",
          showHeader
            ? cn(
          ...
          <div className={cn("w-full", showHeader ? "max-w-2xl mx-auto" : "max-w-2xl mx-auto")}>
            <AIPrompt
